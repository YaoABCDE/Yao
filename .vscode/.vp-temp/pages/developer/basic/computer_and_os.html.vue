...
<p>先来一段计算机导论节选：</p>
<PDF  url="//file.mo7.cc/static/book/basic-computers.pdf" /><blockquote>
<p>原件链接 <a href="https://file.mo7.cc/static/book/basic-computers.pdf" target="_blank" rel="noopener noreferrer">https://file.mo7.cc/static/book/basic-computers.pdf</a></p>
</blockquote>
<p>当然，我知道你一定没耐心看完。</p>
<h3 id="扼要总结" tabindex="-1"><a class="header-anchor" href="#扼要总结"><span>扼要总结</span></a></h3>
...
<li><strong>运算器:</strong> 计算机中执行各种算术和逻辑运算操作的部件，又叫做<code v-pre>算术逻辑部件（ALU）</code>。</li>
<li><strong>控制器:</strong> 计算机的神经中枢，指挥全机中各个部件自动协调工作。由程序计数器、指令寄存器、指令译码器、时序产生器和操作控制器组成，它是发布命令的“决策机构”。</li>
</ul>
<p>现如今为了方便，<code v-pre>运算器</code>和<code v-pre>控制器</code>全部集成到同一个芯片上，统称为<code v-pre>中央处理器(CPU)</code>,其功能是从寄存器中取出指令、解释指令并执行指令。所以下面这张图更符合当下市场情况:</p>
<figure><img src="@source/Yao_Pack/developer/image/computer_basic.png" alt="冯·诺依曼体系结构" tabindex="0" loading="lazy"><figcaption>冯·诺依曼体系结构</figcaption></figure>
...
除此之外如果想让这些部件良好的运行，还需要<a href="https://baike.baidu.com/item/%E7%94%B5%E8%84%91%E6%95%A3%E7%83%AD%E5%99%A8" target="_blank" rel="noopener noreferrer">散热器</a>、<a href="https://baike.baidu.com/item/%E7%94%B5%E8%84%91%E6%9C%BA%E7%AE%B1" target="_blank" rel="noopener noreferrer">机箱</a>。如果想运行一些高画质的游戏，还需要<a href="https://baike.baidu.com/item/%E6%98%BE%E5%8D%A1" target="_blank" rel="noopener noreferrer">GPU</a>。其中主板上会包含 <a href="https://baike.baidu.com/item/HDMI" target="_blank" rel="noopener noreferrer">HDMI</a> 接口，外设控制器，甚至是 <code v-pre>蓝牙</code>和 <code v-pre>Wi-Fi</code> 模块。</p>
<p>如果我们进一步把 CPU、GPU、外设控制器，内存，网络连接模块等硬件功能集中到一块芯片上，那么这块芯片就被称为 <a href="https://baike.baidu.com/item/soc" target="_blank" rel="noopener noreferrer"><code v-pre>SOC</code> (System on a Chip)</a>，如<a href="https://zh.wikipedia.org/wiki/%E9%AB%98%E9%80%9A%E9%A9%8D%E9%BE%8D" target="_blank" rel="noopener noreferrer">高通骁龙</a>、<a href="https://zh.wikipedia.org/wiki/Apple_M1" target="_blank" rel="noopener noreferrer">Apple M1</a>等。</p>
<p>CPU，存储器，外设控制器等模块都是无法独立运行的，模块之间的协作都是需要数据传递和能量消耗的，所以<code v-pre>SOC</code>把这些模块都集中在一个芯片上，信息之间的传递路径就会变得非常短。所以 <code v-pre>SOC</code>的运行能效比传统的 <code v-pre>CPU</code> 要高很多，非常适合电源局促的移动设备。</p>
<blockquote>
<p>这就是<code v-pre>苹果M系列芯片</code>笔记本电脑续航牛逼的核心原因。</p>
</blockquote>
<p>CPU 读取存储器中的程序和数据，完成一系列计算后再传递给外设控制器，然后对应的输出设备再做出相应的反应。我们生活中，大到火箭空间站，小到电视遥控器，路由器，电视机，手机。任何带有芯片控制的机器，其工作原理都基本类似。</p>
<blockquote>
<p>对，没错，任何传统物件只要带上一颗芯片，就能被称为<code v-pre>智能设备</code>，这块芯片如果还能联网，就变成了<code v-pre>物联网</code></p>
</blockquote>
<p>这里有一个视频讲解:</p>
<BiliBili bvid="BV1tM4y1B7qP" /><p>以上的这些部件都被称之为<a href="https://baike.baidu.com/item/%E7%A1%AC%E4%BB%B6" target="_blank" rel="noopener noreferrer">硬件</a>，硬件是软件运行的基础，也决定了一个设备能力的<code v-pre>上限</code>。如果对应的硬件能力不存在，则功能就无法实现。比如你下载了一个音乐播放器，如果扬声器损坏或没有，你也是无法播放音乐的。</p>
...
<p>但是，如果没有对应的软件，再牛逼的芯片也无法发挥作用，就像一个<code v-pre>年轻力壮的植物人</code>一样。<br>
接下来我们要认识一下，世界上最复杂的软件之一: <strong><code v-pre>操作系统</code></strong></p>
<h2 id="二、操作系统初识" tabindex="-1"><a class="header-anchor" href="#二、操作系统初识"><span>二、操作系统初识</span></a></h2>
...
<h3 id="简明扼要" tabindex="-1"><a class="header-anchor" href="#简明扼要"><span>简明扼要</span></a></h3>
...
如最常见的 <a href="https://zh.wikipedia.org/wiki/%E6%AA%94%E6%A1%88%E7%B8%BD%E7%AE%A1" target="_blank" rel="noopener noreferrer">Windows Explorer</a> 就是一种 <code v-pre>图形界面shell</code>；还有 Linux 上默认的 <a href="https://zh.wikipedia.org/wiki/Bash" target="_blank" rel="noopener noreferrer">Bash shell</a>，一般也被称作<code v-pre>文字界面</code>；MacOS 上默认的<a href="https://www.duidaima.com/Group/Topic/OtherTools/17940" target="_blank" rel="noopener noreferrer">Zsh shell</a>；当然还包括 Windows 上的 <a href="https://zh.wikipedia.org/wiki/PowerShell" target="_blank" rel="noopener noreferrer">PowerShell</a>等。</p>
</li>
<li>
...
<BiliBili bvid="BV1G94y1b7C5" /></div></template>
